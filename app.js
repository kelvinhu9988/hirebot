...
/*
 * Response Inconsistency (Variable RIN and True RIN)
 */
var rin = [
	[
...
function generate_scale_table_msg(msg_list) {
  ...
  var msg_to_return = "| ";
  for (var i = 0; i < scale_table.length; ++i) {
    if (msg_list[i] != "" && msg_list[i] != " ")
      msg_to_return += scale_table[i] + ": " + msg_list[i] + " | ";
  }
  return msg_to_return;
}

function generate_critical_items_table_msg(msg_list) {
  var critical_items_table = ["Scale", "Scale Description", "Question", "Answer", "Question Text"];
  var msg_to_return = "| ";
  for (var i = 0; i < critical_items_table.length; ++i) {
    msg_to_return += critical_items_table[i] + ": " + msg_list[i] + " | ";
  }
  return msg_to_return;
}

function generate_summary_msg(msg_list) {
  var msg_to_return = msg_list[0] + ": " + msg_list[1];
  return msg_to_return;
}

/*
 * Calculates the MMPI-2 Test score
 */
function score() {
  var results_to_return = {};

  var scales_to_send = [];
  var critical_items_to_send = [];
  var summaries_to_send = [];
  ...
  var message = "";
	...
	// Columns of the scale and critical item tables
  ...
  message = generate_scale_table_msg(["True", "", t_cnt, "", "", (t_cnt*100/n_real).toPrecision(3)]);
  results_to_send.push(message);

  message = generate_scale_table_msg(["False", "", f_cnt, "", "", (f_cnt*100/n_real).toPrecision(3)]);
  results_to_send.push(message);

  message = generate_scale_table_msg(["?", "", cs_cnt, "", "", (cs_cnt*100/n_real).toPrecision(3)]);
  results_to_send.push(message);
	...
	for (i = 0; i < rin.length; ++i) {
		...
		for(j = 0; j < rin[i][1].length; ++j) {
			...
			if (answers[rp[0]] === rp[1] && answers[rp[2]] === rp[3]) {
				rawscore += rp[4];
			}
		}
		// Append results to the list of messages that are going to be sent later
    message = generate_scale_table_msg(["VRIN", "Variable Response Inconsistency", rawscore, "", rin[i][2+gender][rawscore], ""]);
    scales_to_send.push(message);
	}
    ...
    // The second last one is for male while the last one is for female
		...
		for (j = 0; j < scales[i][1].length; ++j) {
			...
			switch(answers[q]) {
				...
				case "T":
					...
					if (tscale === undefined) {
            message = generate_critical_items_table_msg([scales[i][0][1], scales[i][0][2], q, "True", questions[q]]);
            critical_items_to_send.push(message);
					}
					break;
        // False
				case "F":
		...
		for (j = 0; j < scales[i][2].length; ++j) {
			...
			switch(answers[q]) {
				case "F":
					...
					++rawscore;
					if (tscale === undefined) {
            message = generate_critical_items_table_msg([scales[i][0][1], scales[i][0][2], q, "False", questions[q]]);
            critical_items_to_send.push(message);
					}
					break;
				case "T":
      ...
      message = generate_scale_table_msg([scales[i][0][1], scales[i][0][2], rawscore, kscore === undefined ? " " : kscore, tscore, percent.toPrecision(3)]);
      scales_to_send.push(message);
  ...
  message = generate_summary_msg(["Profile Elevation", pe.toPrecision(3)]);
  summaries_to_send.push(message);
  ...
  message = generate_summary_msg(["Answer Summary", s]);
  summaries_to_send.push(message);

  results_to_return['scales'] = scales_to_send;
  results_to_return['critical_items'] = critical_items_to_send;
  results_to_return['summaries'] = summaries_to_send;
...
function receivedMessage(event) {
  ...
  if (isEcho) {
    ...
    if (quickReplyPayload == "PAYLOAD_TEST_PICKING_TRUE") {
      answers.push("T");
      questionIndex++;
      if (questionIndex < questionLength) {
        sendTestQuestionQuickReply(senderID, questions[questionIndex]);
      } else {
        sendTextMessage(senderID, "Test ended.");
      }
    } else if (quickReplyPayload == "PAYLOAD_TEST_PICKING_FALSE") {
      answers.push("F");
      questionIndex++;
      if (questionIndex < questionLength) {
        sendTestQuestionQuickReply(senderID, questions[questionIndex]);
    ...
    } else if (quickReplyPayload == "PAYLOAD_TEST_PICKING_CANTSAY") {
      answers.push("?");
      questionIndex++;
      if (questionIndex < questionLength) {
        sendTestQuestionQuickReply(senderID, questions[questionIndex]);
    ...
    if (state == "interview") {
      ...
      if (messageAttachment.type == "video") {
        ...
        if (interviewIndex < interview_questions.length) {
          ...
          setTimeout(sendYesOrNoQuickReply, 5000, senderID, "Do you want to proceed to the next question?", "PAYLOAD_INTERVIEW_NEXT_QUESTION_YES", "PAYLOAD_INTERVIEW_NEXT_QUESTION_NO");
        } else {
 ...
 *
 */
function sendTestQuestionQuickReply(recipientId, question) {
  var messageData = {
    recipient: {
      id: recipientId
    },
      ...
      quick_replies: [
        {
          "content_type":"text",
          "title":"True",
          "payload":"PAYLOAD_TEST_PICKING_TRUE"
        },
        {
          "content_type":"text",
          "title":"False",
          "payload":"PAYLOAD_TEST_PICKING_FALSE"
        },
        {
          "content_type":"text",
          "title":"Can't Say",
          "payload":"PAYLOAD_TEST_PICKING_CANTSAY"
        }
      ]
    }
  };
